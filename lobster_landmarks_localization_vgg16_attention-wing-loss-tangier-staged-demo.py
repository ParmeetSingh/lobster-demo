...
landmark_model.compile(loss="mean_squared_error", optimizer="adam")
with open('lobster-demo/landmark-weights-staged30.hd5','rb') as f:
    model_weights = pickle.load(f)
    print("loaded model")
...
#bgr_img = cv2.imread("/tf/data/tangier-visit2/DSC_0492.JPG")
bgr_img = cv2.imread(args["input"])
...
image_temp,corr2 = process_image_keypoints_nobox_util(img_cropped)
...
cv2.imwrite("cropped_predictions_model1.jpg",RGB_img)


# In[33]:


[left,top,ratio] = corr2
global_tuples = []
for i in range(6):
    global_tuples.append((int((tuples[i][0]-left)/ratio+xmin),int((tuples[i][1]-top)/ratio+ymin)))
for i in range(6):
    img = cv2.circle(img,(int(global_tuples[i][0]),int(global_tuples[i][1])), 40, (0,0,255), -1)
print("saving global image")
RGB_img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
cv2.imwrite("predictions_model1.jpg",RGB_img)


# In[ ]:


carapace_start = np.array([(global_tuples[0][0] + global_tuples[1][0])/2.0,(global_tuples[0][1] + global_tuples[1][1])/2.0])
carapace_end = np.array([(global_tuples[2][0] + global_tuples[3][0])/2.0,(global_tuples[2][1] + global_tuples[3][1])/2.0])
tail_end = np.array([(global_tuples[4][0] + global_tuples[5][0])/2.0,(global_tuples[4][1] + global_tuples[5][1])/2.0])
